...
// Change in prod
const redirectUrl = "https://beta.owainfluencers.com/time"
      ...
      .exec((error, item) => {
        ...
        .exec((error, user) => {
          ...
          user.save((error, savedUser) => {
            ...
            if (!item.hallOfFame) {
              request.put({ url:'https://beta.owainfluencers.com/v1/automation/self/stop/', headers:{ 'Content-Type': 'application/x-www-form-urlencoded', 'authorization': req.headers.authorization, 'username': user.username }}, (error, response) => {
                if (error) {
                  console.log(error)
                  return res.status(500).json({ error })
                }

                request.post({ url:'https://beta.owainfluencers.com/v1/automation/self/start/', headers:{ 'Content-Type': 'application/x-www-form-urlencoded', 'authorization': req.headers.authorization, 'username': user.username }}, (error, response) => {
                  if (error) {
                    console.log(error)
                    return res.status(500).json({ error })
                  }
        ...
        if (error) {
          ...
          if (places.meta.error_type === "OAuthAccessTokenException"){
            ...
            .exec((error, user) => {
              ...
              request.get({ url:'https://beta.owainfluencers.com/v1/locations/translate/' + location, headers:{ 'Content-Type': 'application/x-www-form-urlencoded', 'authorization': req.headers.authorization }}, (error, response) => {
                  if (error) {
                    console.log(error)
                    return res.status(500).json({ error })
                  }
    ...
    const { speed, commentForComment } = preferences
      ...
      if (locations.length > 0) {
        ...
        locations.forEach((location) => {
          request.get({ url:'https://beta.owainfluencers.com/v1/locations/translate/' + location.coordinates, headers:{ 'Content-Type': 'application/json', 'authorization': req.headers.authorization }, body: JSON.stringify(location)}, (error, response) => {
              if (error) {
                console.log(error)
                return res.status(500).json({ error })
              }
